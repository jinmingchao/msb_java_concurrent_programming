...
public class StaffTask_tl_1 implements Callable<Boolean> {

    private ThreadLocal<Staff> staff_tl;

    private CountDownLatch cdl;

    public StaffTask_tl_1(ThreadLocal<Staff> staff, CountDownLatch cdl) {
        this.staff_tl = staff;
        this.cdl = cdl;
    }
...
//            staff_tl.remove();
//            staff = staff_tl.get();
//            System.out.println("After calling remove(), the staff's name is " + staff.name);
            return null;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
